...
| Role     | Email                | Password      |
| -------- | -------------------- | ------------- |
| Admin    | admin@dheeraj.com    | Admin2024!    |
| Employee | employee@dheeraj.com | Employee2024! |
| Client   | client@dheeraj.com   | Client2024!   |

## Admin Management

### Creating Additional Admin Users

There are two ways to create additional admin users:

1. **Admin Signup (Protected)** - Requires existing admin authentication:

   ```bash
   POST /api/auth/admin/signup
   Headers: Authorization: Bearer <admin-token>
   Body: { "email": "newadmin@example.com", "password": "password", "name": "Admin Name" }
   ```

2. **Admin Registration (Public with Key)** - Requires admin registration key:
   ```bash
   ...
   Body: {
     ...
     "password": "password",
     "name": "Admin Name",
     "adminKey": "your-admin-registration-key"
   }
   ```

The admin registration key is set via the `ADMIN_REGISTRATION_KEY` environment variable (default: `dheeraj-admin-2024`).
...
| POST   | /api/auth/admin/signup            | Create admin (admin only) |
| POST   | /api/auth/admin/register          | Register admin (with key) |
...
Set `DATABASE_URL`, `JWT_SECRET`, `PORT`, and `ADMIN_REGISTRATION_KEY` environment variables.
