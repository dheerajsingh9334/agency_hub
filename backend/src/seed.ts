...
async function seed() {
  ...
  // Check for --force flag to reset existing users
  const forceReset = process.argv.includes("--force");

  if (forceReset) {
    console.log(
      "🔄 Force reset requested - updating existing test users with new passwords...",
    );

    // Update existing test users with new passwords
  ...
  if (existingAdmin && !forceReset) {
    ...
    console.log(
      "💡 Use 'npm run seed -- --force' to update existing user passwords",
    );
    return;
  }
